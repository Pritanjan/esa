...
const ReplyCard = ({ reply, user, handleLike }) => {
  return (
    <div className="w-full py-3">
      ...
      <div className="ml-12">
        <p className="text-ascet-2 "> {reply?.comment}</p>
...
const PostCard = ({ post, user, deletePost, likePost }) => {
  ...
  return (
    <div className="mb-2 bg-primary p-4 rounded-x1">
      <div className="flex gap-3 items-center mb-2">
        <Link to={"/profile/" + post?.userId?._id}>
          <img
            src={post?.userId?.profileUrl ?? NoProfile}
            alt={post?.userId?.firstName}
            className="w-14 h-14 pbject-cover rounded-full"
          />
        </Link>
        ...
        <div className="w-full  flex justify-between">
          ...
          <span className="text-ascent-2">
            {moment(post?.createdAt ?? "2023-5-25").fromNow()}
          </span>
        </div>
      </div>
            ...
            comments?.map((comment) => (
              <div className="w-full py-2" key={comment?._id}>
                <div className="flex gap-3 items-center mb-1">
                  <Link>
                    <img
                      src={comment?.user?.Id?.profileUrl ?? NoProfile}
